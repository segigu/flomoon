...
**Категория:** feature (service-worker, auto-update, PWA)
**Приоритет:** 🔴 critical
**Сложность:** moderate
...
**Описание:**
Критичная UX проблема: Service Worker кэширует старую версию и пользователь НЕ ВИДИТ обновления после деплоя. Приходилось вручную делать Cmd+Shift+R или лазить в DevTools → Application → Service Workers → Update.

**Что было сделано (5 итераций):**

**v0.3.20** - Первая реализация:
1. ✅ Создан `useServiceWorkerUpdate` hook:
   - `controllerchange` event listener
   - `updateAvailable` state
   - Зеленый UI баннер с кнопкой "Обновить"
2. ✅ Обновлен `serviceWorkerRegistration.ts` - emit `serviceWorkerUpdate` custom event
3. ✅ Включена Service Worker регистрация в `index.tsx`

**v0.3.21** - Добавление версии:
1. ✅ Версия приложения в Settings (`APP_VERSION` display)

**v0.3.22** - Агрессивная стратегия:
1. ✅ `skipWaiting()` в install event (немедленная активация)
2. ✅ Автоматический reload без баннера
3. ✅ Hook упрощен: `controllerchange` → `window.location.reload()`

**v0.3.23** - Backward compatibility:
1. ✅ `updateApp()` → `window.location.reload()` (для старых версий с кнопкой)

**v0.3.24** - ФИНАЛЬНЫЙ FIX:
1. ✅ Немедленный `clients.claim()` в activate event (ДО cache cleanup)
2. ✅ Cache cleanup перемещена в фоновую задачу
3. ✅ Решена проблема "SW stuck in Wait state"

**Root Cause:**
`clients.claim()` блокировался async cache operations → SW застревал в "Wait" состоянии.

**Solution:**
`claim()` выполняется СРАЗУ в начале activate event, cache cleanup запускается в фоне после.

**Коммиты:**
- `26398f6` - v0.3.24
- `2342e2e` - fix: немедленный clients.claim() в activate event
- `36d1281` - v0.3.23
- `6e8f20d` - fix: backward compatibility для кнопки обновления
- `bbc8012` - v0.3.22 (агрессивная стратегия)

**Результат:**
- ✅ Пользователь больше НЕ лазит в DevTools для обновлений
- ✅ Service Worker автоматически берет контроль при новой версии
- ✅ Страница автоматически перезагружается с новым кодом
- ✅ Backward compatibility для версий с кнопкой "Обновить"
