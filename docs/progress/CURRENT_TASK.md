...
**Статус:** 🔄 Переход к Фазе 2 (пропуск задач 1.5-1.8)
...
### Этап 2.1: Настройка Supabase проекта (~2-3 часа)
- [ ] Создать аккаунт на supabase.com
- [ ] Создать проект "flomoon-prod" (регион: Europe)
- [ ] Создать БД схему через SQL Editor (таблицы, RLS, триггеры)
- [ ] Настроить Email Auth в Authentication
- [ ] Записать credentials (Project URL, anon/service keys)

### Этап 2.2: Интеграция Supabase SDK (~2-3 часа)
