...
**Ключевые задачи:**
- [ ] Создать `userProfile.ts` (универсальная структура пользователя)
- [ ] Обновить `types/index.ts` (новые интерфейсы для психологического профиля)
- [ ] Рефакторинг хардкода "Настя" → `getCurrentUser()` во всех файлах
- [ ] Обновить `storage.ts` для работы с профилями
- [ ] Написать тесты для новых утилит
- [ ] Проверить обратную совместимость с localStorage
- [ ] **Версионирование приложения (package.json + UI)** ← НОВОЕ

**Критерии завершения:**
- Нет хардкода "Настя" в коде (только в `userProfile.ts`)
- Все тесты проходят
- Приложение работает без регрессий
- Версия отображается в UI (футер + Settings)

---

### ⏳ Фаза 1.5: Многоязычность (i18n)
...
- [ ] Извлечение и перевод UI текстов (~1290 вхождений кириллицы)
...
- Переключатель языка работает, язык сохраняется
...
**Статус:** ⏳ ОЖИДАЕТ (0% - 0/7 задач)
**Документ:** [PHASE_2_DATABASE_DETAILED_PLAN.md](./roadmap/PHASE_2_DATABASE_DETAILED_PLAN.md)
**ETA:** 12-16 дней

**Цель:** Мигрировать приложение на Supabase (PostgreSQL + Auth + RLS) для поддержки многопользовательского режима с максимальной защитой чувствительных данных. **Изменение архитектуры:** переход на тонкий клиент (только UI в браузере) + толстый сервер (вся бизнес-логика в Edge Functions).

**Ключевые задачи:**
- [ ] Настройка Supabase (таблицы, RLS, auth)
- [ ] Авторизация в UI (login, signup, logout)
- [ ] Миграция storage.ts → storageSupabase.ts
- [ ] Удаление старой синхронизации через GitHub
- [ ] Privacy & Security (Privacy Policy, Delete Account, RLS тесты)
- [ ] Тестирование и документация
- [ ] **Миграция логики на Edge Functions (тонкий клиент)** ← НОВОЕ

**Критерии завершения:**
- Supabase настроен и работает
- Пользователи могут регистрироваться и логиниться
- Данные Насти мигрированы из localStorage
- RLS policies гарантируют privacy (user A не видит данные user B)
- Privacy Policy и Delete Account реализованы (GDPR)
- **Edge Functions работают (cycleUtils, aiClient на сервере)** ← НОВОЕ
- **API ключи Claude/OpenAI хранятся в Supabase Secrets (не в .env клиента)** ← НОВОЕ

---

### ⏳ Фаза 3: AI-агенты и сбор данных
**Статус:** ⏳ ОЖИДАЕТ (0% - 0/14 задач)
**Документ:** [PHASE_3_AI_AGENTS.md](./roadmap/PHASE_3_AI_AGENTS.md)
**ETA:** 5-6 дней
...
### ⏳ Фаза 4: Персонализация промптов
**Статус:** ⏳ ОЖИДАЕТ (0% - 6 задач)
**Документ:** [PHASE_4_PERSONALIZATION.md](./roadmap/PHASE_4_PERSONALIZATION.md)
**ETA:** 3-4 дня
...
### ⏳ Фаза 5: UI и аналитика
**Статус:** ⏳ ОЖИДАЕТ (0% - 10 задач)
**Документ:** [PHASE_5_UI.md](./roadmap/PHASE_5_UI.md)
**ETA:** 4-5 дней
...
**Общий прогресс:** 6% (3/54 задачи)
**Общий ETA:** ~28-37 дней работы

**⚠️ Обновления (2025-10-26):**
- Добавлена задача 1.9 (версионирование) в Фазу 1
- Добавлена Фаза 1.5 (многоязычность i18n) - 2-3 дня
- Добавлена задача 2.7 (миграция на Edge Functions) в Фазу 2
...
- **[PHASE_1.5_I18N_DETAILED_PLAN.md](./roadmap/PHASE_1.5_I18N_DETAILED_PLAN.md)** - многоязычность (i18n)
- **[PHASE_2_DATABASE_DETAILED_PLAN.md](./roadmap/PHASE_2_DATABASE_DETAILED_PLAN.md)** - база данных и авторизация
- **[PHASE_3_AI_AGENTS.md](./roadmap/PHASE_3_AI_AGENTS.md)** - агентская система
- **[PHASE_4_PERSONALIZATION.md](./roadmap/PHASE_4_PERSONALIZATION.md)** - персонализация AI
- **[PHASE_5_UI.md](./roadmap/PHASE_5_UI.md)** - пользовательский интерфейс
